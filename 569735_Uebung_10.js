/**
 * @file GTAT2 Game Physics Übung 10
 * @copyright Pauline Röhr 09.01.2021, Matrikelnummer: 569735
 */
...
function draw() {
    ...
    if (gameState == gameStateNew) {
        ...
        positionSlider = createSlider(-distanceTriangles / 2 * 100, distanceTriangles / 2 * 100, 0, 1);
        positionSlider.position(8 * windowWidth / 10, 6.5 * windowHeight / 10);
        positionSlider.attribute("hidden", "hidden");
    ...
    if (gameState == gameStateTest) {
        textSize(16);
        fill('#0000BB');
        text('Position: ' + positionSlider.value() + "cm", 8 * windowWidth / 10, 6.5 * windowHeight / 10 + 40);
    ...
    if (gameState == gameStateTest) {
        fill('#FFAA77');
        circle(positionSlider.value() / 100 * scaleM, scaleM * radiusMiddleBall, scaleM * 2 * radiusMiddleBall);
        return;
    }
...
function startTestModus() {
    if (gameState != gameStateActive && gameState != gameStateTest) {
        gameState = gameStateTest;
        testButton.html('RESET');
        button.attribute("disabled", "disabled");

        positionSlider.removeAttribute("hidden");
